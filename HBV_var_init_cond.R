...
HBV <- function(pars,initcond,P,Temp,PET,routing = 0){
  ...
  SWE[1] <- initcond[1] 
  SUZ    <- initcond[2]      #Initial upper zone storage
  SLZ    <- initcond[3]      #Initial lower zone storage
  SP     <- initcond[4]      #initial value for simulated snowpack 
  WC     <- initcond[5]      #Initial liquid water in snowpack 
  SM     <- initcond[6]      #Initial soil storage content
  ...
  if (routing == 1){ 
    ...
    if (MAXBAS %% 1 > 0){                                                          #allow base of weighting function to be noninteger, adjusts for extra weights for the last day
    ...
    for (k in 2:length(I)){                                                         #integration of function
      MAXBAS_w[k] = sum(h[floor(I[k-1]):floor(I[k])]) 
    }
    
    MAXBAS_w <- MAXBAS_w[2:end] / sum(MAXBAS_w[2:end])                            #make sure integration sums to unity for mass balance
